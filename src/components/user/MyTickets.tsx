...
interface MyTicketsProps {
  ...
  currentUser?: { name: string; email: string }; // optional
...
const MyTickets: React.FC<MyTicketsProps> = ({
  ...
  // Safe defaults for layout
  const safeUser = currentUser || { name: "Guest", email: "guest@example.com" };
  const safeLogout = onLogout || (() => console.log("Logout clicked"));
  ...
  return (
    <UserLayout 
      ...
      title="My Tickets" // Added title prop here
    >
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-blue-600">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Ticket #</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-gray-50 cursor-pointer">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{ticket.createdAt.toLocaleDateString()} {ticket.createdAt.toLocaleTimeString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
