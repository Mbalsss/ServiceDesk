        ...
        <tbody className="divide-y divide-gray-200">
          {filteredTickets.map(ticket => (
            <tr key={ticket.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="py-4 px-4 text-sm text-gray-500">{formatDate(ticket.created_at)}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const TicketsSection = () => {
    ...
    if (error) return (
      <div className="p-8 text-center">
        ...
        <button onClick={fetchTickets} className="px-4 py-2 bg-gray-900 text-white rounded-md hover:bg-gray-800">
          Retry
        </button>
      </div>
    );
